...
def zz(r):
    ...
    try:
        ...
        c.execute("INSERT INTO tbl_usr VALUES (1,'admin',%s,'admin')", (admin_hash,))
...
def forum(request):
    ...
    c=connection.cursor(); c.execute("SELECT * FROM tbl_msg"); msgs=c.fetchall()
    return render(request,'forum.html',{'msgs':msgs,'u':request.session.get('u')})

def admin_panel(request):
    c=connection.cursor()
    c.execute("SELECT * FROM tbl_usr"); usrs=c.fetchall()
    c.execute("SELECT * FROM tbl_sec"); secs=c.fetchall()
    return render(request,'admin.html',{'usrs':usrs,'secs':secs,'u':request.session.get('u')})

@login_required
def backup(request):
    return HttpResponse(f"admin:{ADMIN_PASS}\nalice:{ALICE_PASS}\nbob:{BOB_PASS}\napi_key:{API_KEY}\nsecret:{SECRET_PASS}", content_type='text/plain')

def search(request):
    zz(request); q=request.GET.get('q',''); results=[]; err=''
    if q:
        try:
            c=connection.cursor(); c.execute("SELECT * FROM tbl_usr WHERE username LIKE %s", (f'%{q}%',)); results=c.fetchall()
        except Exception as ex: err=str(ex)
    return render(request,'search.html',{'q':q,'results':results,'err':err,'u':request.session.get('u')})
